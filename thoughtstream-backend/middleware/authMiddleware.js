...
export const ensureAuthenticated = (req, res, next) => {
  // Returns true if user is authenticated (has a valid session)
  if (req.isAuthenticated && req.isAuthenticated()) {
    // User is authenticated, proceeds with request 
    return next();
  }
  // User not authenticated, 401 Unauthorized response returned.
