...
async function fetchCurrentWeather(location, API_KEY) {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(data.message || "Failed to fetch current weather data. Try again");
        }
        ...
        throw new Error(error.message || "Failed to fetch current weather data. Try again");
    }
}
